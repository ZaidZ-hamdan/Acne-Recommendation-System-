"""
Use the trained model to classify each image in Unlabeled folders,
then move it into the matching Level1/Level2/Level3/Level4 folder.
Run from CRS folder. Requires trained model in saved_model/.
"""
import re
import shutil
...
def main():
    ...
    if model is None:
        ...
        return
    print("Model loaded. Classifying Unlabeled images and moving to level folders.\n")
    ...
    for split in SPLITS:
        ...
        if not unlabeled_dir.is_dir():
            continue
        images = [f for f in unlabeled_dir.iterdir() if f.is_file() and f.suffix.lower() in IMAGE_EXTS]
        if not images:
            continue
        ...
        for path in images:
            ...
            indices[level] += 1
            moved[level] += 1
        print(f"{split}/Unlabeled: moved {len(images)} images -> Level1:{moved['Level1']} Level2:{moved['Level2']} Level3:{moved['Level3']} Level4:{moved['Level4']}")
    print("\nDone. You can delete the empty Unlabeled folders if you want.")
